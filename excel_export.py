...
def _build_registry_sheet(wb, registry, flags, flag_titles, flag_emoji, metric_labels):
    ...
    rename = {**metric_labels,
              **{k: (f"{flag_emoji.get(k,'')} {flag_titles.get(k,k)}").strip() for k in flags.columns}}
    ...
    flag_display = [(f"{flag_emoji.get(k,'')} {flag_titles.get(k,k)}").strip() for k in flags.columns]
...
def _build_flag_sheet(wb, registry, flags, df_raw, month_info, flag_name, flag_titles):
    ...
    sort_keys = {
        ...
        'тренд_снижение': 'trend_annual_drop',
        'падение_год_к_году': 'yoy_median_ratio',
        'нулевые_показания': 'zero_share',
        'круглые_числа': 'round_share',
        'ml_аномалия': 'iso_score',
    }
    sk = sort_keys.get(flag_name)
    if sk and sk in sub.columns:
        ascending = flag_name == 'падение_год_к_году'
        sub = sub.sort_values(sk, ascending=ascending)
    ...
    cols_show = ['Вид потребителя', 'Заводской номер прибора учета',
                 ...
                 'trend_annual_drop', 'yoy_median_ratio', 'zero_share',
                 'round_share', 'iso_score',
                 'longest_silence', 'longest_flat_run', 'power_ratio',
                 'cohort_z', 'сезонность', 'кол-во_флагов', 'риск_балл']
...
def _build_settings_sheet(wb, config, report):
    ...
    config_labels = {
        ...
        'trend_annual_drop_pct': 'Тренд: годовое снижение (доля)',
        'trend_window_months': 'Тренд: окно, мес.',
        'trend_min_mean_kwh': 'Тренд: мин. среднее, кВт·ч',
        'yoy_drop_pct': 'Год к году: порог падения (доля)',
        'yoy_months': 'Год к году: сравниваемых месяцев',
        'yoy_min_pairs': 'Год к году: мин. пар месяцев',
        'zero_share_threshold': 'Нули: порог доли нулевых месяцев',
        'zero_min_nonzero_kwh': 'Нули: мин. ненулевое среднее, кВт·ч',
        'round_base': 'Круглые: кратность',
        'round_share_threshold': 'Круглые: порог доли',
        'round_min_values': 'Круглые: мин. значений',
        'iso_contamination': 'ML: доля аномалий (contamination)',
        'risk_weights': 'Веса флагов для риск-балла',
    }
    ...
    if active_flags:
        for n in active_flags:
            ws.cell(row=r, column=2,
                    value=(f"  {flag_emoji.get(n,'')} {flag_titles.get(n, n)}").rstrip()).font = Font(name='Calibri', size=11, color=RED, bold=True)
